...
Locate the project root the same way the periodic-status skills do: check the current
working directory, then `./project/` beneath it, then walk parent directories, for
`project-config.md`. Read `evidence-notes-current.md` and `project-state.md` from that root. For iterations in design or discovery phases (I5 A, I5 B, I6, I7), meeting notes and emails are typically the **primary** source — there may be few or no GitHub commits for these. Capture session content, decisions, and outcomes with specificity matching the Apr 27 and May 27 approved examples.
